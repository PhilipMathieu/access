...
def convert_csv_to_parquet(
    ...
    # Determine output path
    output_path = input_path.with_suffix(".parquet") if output_path is None else Path(output_path)
...
def main():
    ...
    parser.add_argument("input", type=Path, help="Input file or directory to convert")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--recursive", action="store_true", help="Recursively convert all files in directory"
    )
    parser.add_argument(
        "--csv", action="store_true", help="Convert CSV files instead of shapefiles"
    )
    ...
    if input_path.is_file():
        ...
        if args.csv or input_path.suffix == ".csv":
            success = convert_csv_to_parquet(input_path, args.output, args.overwrite)
        else:
            success = convert_shapefile_to_geoparquet(input_path, args.output, args.overwrite)
        ...
        # Convert directory
        files = find_csv_files(input_path) if args.csv else find_shapefiles(input_path)
        ...
        for file_path in files:
            if args.csv or file_path.suffix == ".csv":
                success = convert_csv_to_parquet(file_path, None, args.overwrite)
            else:
                success = convert_shapefile_to_geoparquet(file_path, None, args.overwrite)
