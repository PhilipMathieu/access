...
from probe_data_sources import (
    DATA_SOURCES,
    load_metadata,
    save_metadata,
    get_version_from_url,
    get_local_file_date,
    get_remote_file_date
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('data/update_log.txt'),
                ...
                if chunk:
                    ...
                    if total_size > 0:
                        ...
                        if downloaded % (chunk_size * 100) == 0:  # Log every 100 chunks
                            logging.info(f"Downloaded {percent:.1f}% ({downloaded}/{total_size} bytes)")
...
def main():
    ...
    parser.add_argument(
        "--source",
        type=str,
        help="Update a specific source by name"
    )
