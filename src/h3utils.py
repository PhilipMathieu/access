...
def h3_merge(df, reln=None, inplace=False, resolution=None, region_config=None):
    ...
    if reln is None:
        if region_config:
            reln_path = (
                region_config.data_root
                / "blocks"
                / f"tl_2020_{region_config.state_fips}_tabblock20_h3_{resolution or DEFAULT_H3_RESOLUTION}.csv"
            )
        else:
        ...
        reln = pd.read_csv(
            str(reln_path), converters={"GEOID20": str, "h3_fraction": float, "h3id": str}
        )
    if inplace:
...
def h3_plot(df, col: str, lognorm=True, inplace=False, **plot_kwargs):
    ...
    return hexes.h3.h3_to_geo_boundary().plot(col, norm=norm, label=col, **plot_kwargs)
...
def h3_to_h3t(df, filename=None, inplace=False):
    ...
    json_dict = {
        "metadata": {"note": "created with h3utils.py"},
