...
def convert_file(
    input_path: Path, output_path: Path, layer_name: str, min_zoom: int = 0, max_zoom: int = 14
) -> bool:
    """
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Convert shapefiles and GeoJSON to PMTiles format")
    parser.add_argument("input", type=Path, help="Input shapefile or GeoJSON file")
    parser.add_argument("output", type=Path, help="Output PMTiles file path")
    parser.add_argument("-l", "--layer", required=True, help="Layer name for the PMTiles")
