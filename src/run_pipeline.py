...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data/pipeline_log.txt"), logging.StreamHandler()],
)
...
def run_pipeline(
    ...
    if not skip_h3 and success:
        ...
        try:
            ...
            h3_output = Path(
                f"data/blocks/tl_2020_{region_config.state_fips}_tabblock20_h3_{DEFAULT_H3_RESOLUTION_AREA}.parquet"
            )
...
def main():
    ...
    parser.add_argument("--state", default="Maine", help="State name (default: Maine)")
    parser.add_argument(
        "--census-api-key", help="Census API key (default: read from CENSUS_API_KEY env var)"
    )
    parser.add_argument(
        "--skip-walk-times", action="store_true", help="Skip walk time calculation step"
    )
    parser.add_argument("--skip-merging", action="store_true", help="Skip merging step")
    parser.add_argument(
        "--skip-analysis", action="store_true", help="Skip analysis step (census/CEJST)"
    )
    parser.add_argument("--skip-visualization", action="store_true", help="Skip visualization step")
    parser.add_argument("--skip-h3", action="store_true", help="Skip H3 processing step")
    parser.add_argument(
        "--n-jobs",
        type=int,
        default=-1,
