...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('data/processing_log.txt'),
...
def process_updated_data_sources(sources: Optional[List[str]] = None) -> Dict[str, bool]:
    ...
    if sources:
        ...
        sources_to_process = [
            name for name, source_metadata in metadata.items()
...
def main():
    ...
    parser.add_argument(
        "--sources",
        nargs="+",
        help="Specific sources to process (default: all updated sources)"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
