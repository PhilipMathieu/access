...
def setup_fonts(font_path: str | Path | None = None) -> None:
...
def plot_access_by_group(
    ...
    **kwargs,
    ...
    percents_std = (
        np.sqrt(p * (1 - p) * 100).melt(ignore_index=False, value_name="std").reset_index()
    )
    ...
    if len(std_values) != len(ax.patches):
        ...
        std_dict = dict(
            zip(
                zip(percents[groupby_col], percents["variable"], strict=False),
                percents["std"],
                strict=False,
            )
        )
    ...
    widths = 0.5 * np.tile(
        np.sqrt(g.sum()["P1_001N"] / g.sum()["P1_001N"].sum()).to_numpy().reshape(-1, 1), p.shape[1]
    )
    for bar, newwidth in zip(ax.patches, widths.ravel(), strict=False):
        x = bar.get_x()
        width = bar.get_width()
        center = x + width / 2.0
        bar.set_x(center - newwidth / 2.0)
...
def plot_nearest_lands(
    ...
    **kwargs,
    ...
    data_filtered = data[data["ALAND20"] > 0] if "ALAND20" in data.columns else data

    # Create melted dataframe with nearest walk time
    melt = data_filtered.melt(id_vars=id_vars, value_vars=cols_bools)
    ...
    melt = gpd.GeoDataFrame(
        melt.groupby(groupby_col).agg(
            {"geometry": "first", "nearest": "min", disadvantage_col: "max"}
        )
    ).set_crs("EPSG:3857")
    ...
    melt[melt[disadvantage_col]].plot(
        ax=ax, facecolor="none", edgecolor="crimson", hatch="//", linewidth=0
    )
...
def generate_all_figures(
    ejblocks_path: str | Path,
    output_dir: str | Path = "../figs/",
    trip_time_cols: list[str] | None = None,
