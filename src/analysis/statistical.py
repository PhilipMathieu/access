...
def analyze_access_disparity(
    ...
    access_var = ~data[access_col] if invert_access else data[access_col]

    # Create contingency table weighted by population
    tab = pd.crosstab(data[disadvantage_col], access_var, data[population_col], aggfunc="sum")
...
def calculate_population_metrics(
    ...
    if groupby_col:
        ...
        g = data.groupby(lambda _: True)[[population_col] + cols_pops]
