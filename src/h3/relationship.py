...
def calculate_h3_fractions(
    ...
    blocks['h3_boundary'] = blocks.progress_apply(
        lambda row: get_boundary_tiles(row, resolution),
        axis=1
    )
    ...
    if method == "area":
        ...
        def calculate_area_fraction(row):
            h3ids = pd.DataFrame.from_records(
                [{'fraction': 0, 'h3id': tile} for tile in row['h3ids']],
                index='h3id'
            )
            h3ids = h3ids.h3.h3_to_geo_boundary().to_crs(DEFAULT_CRS)
            h3ids['fraction'] = (
                h3ids['geometry'].intersection(row['geometry']).area /
                row['geometry'].area
            )
        ...
        def calculate_population_fraction(row):
            ...
            h3ids = pd.DataFrame(
                {'POPULATION': [population for tile in row['h3ids']]},
                index=row['h3ids']
            )
            h3ids = h3ids.h3.h3_to_geo_boundary().to_crs(DEFAULT_CRS)
            fraction = (
        ...
        result = result[['h3id', 'population_fraction']].rename(columns={'population_fraction': 'h3_fraction'})
        return result
