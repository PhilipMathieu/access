...
def find_cejst_shapefile(cejst_dir: Path) -> Path | None:
...
def get_state_fips_code(state_name: str) -> str | None:
...
def crop_cejst_to_state(
    ...
    try:
        ...
        if fips_code_column in gdf.columns:
            ...
            logging.warning(
                f"Neither '{fips_code_column}' nor '{state_fips_column}' found in columns"
            )
            logging.info("Searching for state-related columns...")

            # Look for columns that might contain state info
            state_cols = [
                col for col in gdf.columns if "state" in col.lower() or "fips" in col.lower()
            ]
            if state_cols:
                ...
                for col in state_cols:
                    ...
                    try:
                        ...
                        if len(test_filter) > 0:
                            ...
                            logging.info(
                                f"Filtered using column '{col}': {len(filtered_gdf)} tracts"
                            )
                            break
  ...
  python crop_cejst_to_state.py --input data/cejst-us/cejst.shp --output data/cejst-maine.shp
    ...
    if args.input:
        ...
        if not shapefile:
            ...
            logging.info(
                "Please run: python src/update_data_sources.py --source 'CEJST (Climate Equity and Justice Screening Tool)'"
            )
            return 1
        cejst_shapefile = shapefile
