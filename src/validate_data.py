...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('data/validation_log.txt'),
...
def get_schema(file_path: Path) -> Dict:
    ...
    try:
        ...
        schema = {
            ...
            "geometry_type": gdf.geometry.geom_type.unique().tolist() if hasattr(gdf.geometry, 'geom_type') else None
        }
...
def compare_schemas(old_schema: Dict, new_schema: Dict) -> Dict:
    ...
    if old_count != new_count:
        changes["row_count_change"] = {
            ...
            "percent_change": ((new_count - old_count) / old_count * 100) if old_count > 0 else None
        }
...
def validate_data_quality(file_path: Path, schema: Optional[Dict] = None) -> Dict:
...
def validate_data_file(file_path: Path, source_name: Optional[str] = None) -> Dict:
    ...
    if source_name:
        ...
        if source_schemas:
            ...
            latest_version = max(source_schemas.keys(), key=lambda x: source_schemas[x].get("timestamp", ""))
    ...
    high_missing = {
        col: metrics for col, metrics in quality_metrics["missing_values"].items()
...
def main():
    ...
    parser.add_argument(
        "--source",
        type=str,
        help="Validate a specific source by name"
    )
    parser.add_argument(
        "--all",
        action="store_true",
